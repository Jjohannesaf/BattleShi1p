...
export default function Gameboard() {
    ...
    function winGame() {
        if (SHIPS === 0) {
            return setStatus('You trashed all the ships');
        } else {
            return '';
        }
    }
    ...
    function checkWinner() {
        if (shipAmount === 0) {
            ...
            setStatus('You trashed all the ships');
            SetStart(false);
        } else if (bombsLeft === 0) {
            setStatus('Game over! You ran out of bombs!');
            SetStart(false);
            clearInterval(timerRef.current);
        } else if (seconds === 0) {
            clearInterval(timeRef.current);
            setStatus('Game Over.You run out of time!');
            SetStart(false);
        } else {
            return '';
        }
    ...
    return (
        <View style={styles.gameboard}>
            ...
            <Text style={styles.gameinfo}>Time left:{seconds}</Text>
